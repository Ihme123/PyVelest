...
class main():
    ...
    def mk_syn_model(self,inp_syntvel=path.join('par','syntvel.dat')):
        ...
        plt.tight_layout()
        plt.savefig(path.join('figs',self.project_nm,"synt_models_"+self.project_nm+'.png'))
        plt.close()
    ...
    def analyse_velest_res(self):
        ...
        if self.synthvel_files:
            ...
            for nm in self.p_good_models:
                y = [row[0] for row in self.avg_abs_adj[nm]]     # len = self.nit+1
                x = range(1, len(y)+1)                           # len matches y
                ax1.plot(x, y, marker='o', ms=3, lw=.75, color='grey')
                self.minmod[nm] = mean(y)

            self.ind   = min(self.minmod, key=self.minmod.get)
            y_min      = [row[0] for row in self.avg_abs_adj[self.ind]]
            x_min      = range(1, len(y_min)+1)
            ax1.plot(x_min, y_min, marker='o', ms=3, lw=.75, color='red', label='Minimum model, %d' % self.ind)
            ...
            self.minmod = {}
            for nm in self.p_good_models:
                y = [row[1] for row in self.avg_abs_adj[nm]]   # take the column you need
                x = range(1, len(y) + 1)                       # now x and y match
                ax2.plot(x, y,marker='o', ms=3, lw=.75, color='red',label='Minimum model,%d'%(self.ind))
            ...
            self.minmod = {}
            for nm in self.p_good_models:
                 y = [row[2] for row in self.avg_abs_adj[nm]]   # take the column you need
                 x = range(1, len(y) + 1)                       # now x and y match
                 ax3.plot(x, y, marker='o', ms=3, lw=.75, color='red',label='Minimum model,%d'%(self.ind))
            ...
            self.minmod = {}
            for nm in self.p_good_models:
                y = [row[3] for row in self.avg_abs_adj[nm]]   # take the column you need
                x = range(1, len(y) + 1)                       # now x and y match
                ax4.plot(x, y, marker='o', ms=3, lw=.75, color='red',label='Minimum model,%d'%(self.ind))
            ...
            self.test = {}
            for nm in self.p_good_models:
                y = [row[4] for row in self.avg_abs_adj[nm]]   # take the column you need
                x = range(1, len(y) + 1)                       # now x and y match
                ax5.plot(x, y, marker='o', ms=3, lw=.75, color='red',label='Minimum model,%d'%(self.ind))
            ...
            plt.tight_layout()
            plt.savefig(path.join('figs',self.project_nm,'velest_analytics_'+self.project_nm+'.png'))
            plt.close()
            ...
            if len(orms):           # Initial
                datasets.append(orms)
                colors.append('r')
                labels.append('Initial')

            if len(mrms):           # Relocated
                datasets.append(mrms)
                colors.append('b')
                labels.append('Relocated')

            if datasets:            # plot only if something to show
                ax6.hist(datasets, color=colors, label=labels,
                         histtype='bar', bins=bns, rwidth=0.8, edgecolor='gray')
            ...
            plt.tight_layout()
            plt.savefig(path.join('figs',self.project_nm,'velest_final_'+self.project_nm+'.png'))
            plt.close()
    ...
    def plot_data(self):
        ...
        plt.tight_layout()
        plt.savefig(path.join('figs',self.project_nm,'data_'+self.project_nm+'.png'))
        plt.close()
...
"""
    ...
    def plot_path(self):
        ...
        plt.savefig(path.join('figs',self.project_nm,'path_'+self.project_nm+'.png'))
        plt.close()
    ...
    def plot_final(self):
        ...
        plt.savefig(path.join('figs',self.project_nm,'final_'+self.project_nm+'.png'))
        plt.close()
